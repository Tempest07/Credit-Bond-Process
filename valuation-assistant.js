...
function table(evidence, result) {
  ...
  return `<div class="va-table"><table><thead><tr><th>可比券</th><th>期限</th><th>估值</th><th>调整</th><th>权重</th></tr></thead><tbody>${evidence.candidates.map(c => {
    ...
    return `<tr><td>${escape(c.shortName)}<span>${escape(c.source)} · ${escape(c.yieldBasis)} · ${escape(c.valuationDate)}</span></td><td>${Number(c.years).toFixed(2)}Y</td><td>${rate(c.rate)}</td><td>${s ? `${(s.tenorAdjustmentBp+s.marketAdjustmentBp).toFixed(2)} bp` : '未采用'}</td><td>${s ? `${(s.weight*100).toFixed(1)}%` : '—'}</td></tr>${s || excluded.has(c.securityId) ? `<tr class="va-reason"><td colspan="5">${escape(s?.rationale || excluded.get(c.securityId))}${s ? ` · 折算 ${rate(s.adjustedYield)}` : ''}</td></tr>` : ''}`;
...
function resultCard(r) {
  ...
  return `<article class="va-card"><div class="va-color"><div class="va-hero"><div><div>${escape(t.durationText)} 参考估值</div><strong class="va-rate">${r.center === null ? '暂无可靠建议' : rate(r.center)}</strong></div><div class="va-range"><span>参考区间</span><strong>${r.low === null ? '依据不足，未给区间' : `${rate(r.low)} — ${rate(r.high)}`}</strong></div></div><div class="va-profile"><span>${escape(e.target.profile?.bondClass || '')} · ${escape(e.target.venue || e.target.profile?.market)} · ${offering}</span><span>模型研判</span></div></div>
    ...
    <details><summary>可比券</summary>${table(e,r)}${e.excluded.length ? `<div class="va-text">${e.excluded.map(c => `${escape(c.shortName)}：${escape(c.reason)}`).join('<br>')}</div>` : ''}</details>
    <details><summary>定价依据与调整</summary><div class="va-text"><ul>${r.reasons.map(s=>`<li>${escape(s)}</li>`).join('')}</ul>${r.comparables.map(c=>`<div class="va-adjust">${escape(e.candidates.find(b=>b.securityId===c.securityId)?.shortName)}：期限 ${c.tenorAdjustmentBp.toFixed(2)} bp，市场 ${c.marketAdjustmentBp.toFixed(2)} bp</div>`).join('')}${r.limitations.length ? `<ul>${r.limitations.map(s=>`<li>${escape(s)}</li>`).join('')}</ul>`:''}<div>已核验引用和算术；理由仍需复核，区间不是统计置信区间。</div></div></details>
