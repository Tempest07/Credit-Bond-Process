import { normalizeProjectRecord, trancheNeedsPayment } from "./lifecycle.js?v=20260708-reminder-center";
import { protocolTransferTodos } from "./protocol-transfer.js?v=20260708-reminder-center";
...
export function buildUnifiedReminders(state = {}, referenceDate = new Date()) {
  ...
  for (const item of protocolTransferTodos(state.protocolTransfers || [], referenceDate)) {
    ...
    reminders.push({
      ...
      detail: `${item.record.shortName || item.record.code || "协议转让"} · ${formatDueDate(item.step.dueDate, today, tomorrow)}`,
...
function collectProjectBidReminders(reminders, project, referenceDate) {
  ...
  reminders.push({
    ...
    title,
    detail: `${project.shortName || "未命名项目"} · ${formatDateTime(project.cutoffAt)}`,
...
function collectProjectPaymentReminders(reminders, project, referenceDate, today, tomorrow) {
  for (const tranche of project.tranches || []) {
    ...
    reminders.push({
      ...
      detail: `${tranche.shortName || project.shortName || "未命名品种"} · ${paymentTiming.detail}`,
