...
export function buildBidResultSummary(project) {
  ...
  const unit = project.pricingUnit || "万元";
  const pricingText = project.comprehensivePricing
    ? `综合定价：扣税后营收${formatValue(project.afterTaxRevenue, unit)}，FTP${formatValue(project.ftpCost, unit)}，扣除FTP后收益${formatValue(project.netIncome, unit)}。`
    : "";
...
function formatValue(value, unit) {
  return Number.isFinite(numberOrNull(value)) ? `${value}${unit}` : "待补";
}
