...
**2026-09-03 检查时线上尚无 AI 绑定，当前功能尚未发布；生产仍为 `007bba3` / build `3.3.0.31`。** 本分支准备的 build 为 `3.3.0.32`，缓存标记 `20260903-semantic-issuance`。配置完成、验收通过后才推送 main，以免替换原功能后线上无法调用。
...
其余 8 个正例保持相同的单次提取输入与校验规则，已在上一轮通过；这里不是声称另跑了一轮 12/12，也不是生产总体准确率统计。UI 另一次实际识别苏元禾 A/B 成功。上线仍须补齐 AI 绑定并验证发布。
